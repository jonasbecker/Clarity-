...
//   id, user_id, title, area, due, done, inserted_at
...
export async function createTask({ title, area, due }) {
  const { data, error } = await supabase
    .from(TABLE)
    .insert({ title, area, due, done: false })
    .select('id, title, area, due, done')
    .single()
  if (error) throw error
  return data
